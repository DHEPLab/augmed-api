...
def test_send_email_failed_when_template_not_found(mocker):
    ...
    with pytest.raises(
        BusinessException,
        match=re.compile(BusinessExceptionEnum.RenderTemplateError.name),
    ):
...
def test_send_email_failed(mocker):
    ...
    with pytest.raises(
        BusinessException, match=re.compile(BusinessExceptionEnum.SendEmailError.name)
    ):
        send_email(SUBJECT, TO_ADDRESSES, html_template_name, **data)
...
def test_send_email_success(mocker):
    ...
    mock_client = mocker.Mock()
    mock_client.send_email.return_value = {"MessageId": "email_id"}
    mocker.patch("boto3.session.Session.client", return_value=mock_client)
