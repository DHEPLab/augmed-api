...
@pytest.fixture()
def test_config():
    return [{"type": "Text", "title": "Please make a diagosis"}]
...
def test_add_answer_config_success(client, mocker, test_config):
    ...
    assert {"data": {"id": str(id)}, "error": None} == response.json
...
def test_add_answer_config_failed_by_empty_config(client):
    ...
    assert {
        ...
        "error": {
            "code": "1020",
            "message": "Invalid answer config. Error:Answer config can not be empty list.",
        },
    } == response.json
...
def test_get_latest_answer_config_success(client, mocker, test_config):
    ...
    assert response.status_code == 200
    assert {"data": answer_config.to_dict(), "error": None} == response.json


def test_get_latest_answer_config_failed_by_no_answer_config(
    client, mocker, test_config
):
    mocker.patch(
        "src.configration.service.answer_config_service.AnswerConfigurationService.get_latest_answer_config",
