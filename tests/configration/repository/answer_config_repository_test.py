...
def test_add_answer_config(answer_config_repository):
    config = AnswerConfig(
        config={"type": "MultipleChoice", "title": "title", "options": ["A", "B"]}
    )
...
def test_query_latest_answer_config(answer_config_repository):
    ...
    config3 = AnswerConfig(
        config={"type": "SingleChoice", "title": "title", "options": ["A", "B"]}
    )
...
def test_get_answer_config(answer_config_repository):
    ...
    assert (
        answer_config_repository.get_answer_config(not_exist_answer_config_id) is None
    )
