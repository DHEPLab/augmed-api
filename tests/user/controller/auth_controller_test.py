...
def mock_auth_service(mocker):
    ...
    mocker.patch("src.user.service.auth_service.AuthService.update_password")
...
def pilot_user_request(test_user: User):
    return SignupRequest(email=test_user.email, password="9eNLBWpws6TCGk8_ibQn")
...
def test_sign_up_request_invalid(client, test_user: User):
    UserRepository(db.session).create_user(test_user)

    signup = {"email": "john@exmaple.com", "password": "simplepassword"}

    response = client.post(
        "/api/auth/signup", data=json.dumps(signup), content_type="application/json"
    )
...
def test_sign_up_success(client, test_user: User):
    UserRepository(db.session).create_user(test_user)

    signup = {"email": "john@exmaple.com", "password": "9eNLBWpws6TCGk8_ibQn"}
    ...
    assert {"data": "Sign up successfully", "error": None} == response.json


def test_sign_up_failed_when_user_is_not_in_pilot(client, test_user: User):
    user = test_user.copy()
    UserRepository(db.session).create_user(user)

    signup = {"email": "not-in-pilot@exmaple.com", "password": "9eNLBWpws6TCGk8_ibQn"}
...
def test_sign_up_failed_when_user_aready_sign_up(client, test_user: User):
...
def test_reset_password_request_failed_with_no_user(client, mocker):
    ...
    assert {
        ...
        "error": {
            "code": "1040",
            "message": "Email failed to send. Please try again. If the problem persists, contact dhep.lab@gmail.com for support.",
        },
    } == response.json


def test_update_password(client, mock_auth_service):
    request = {"resetToken": "token", "password": "password123"}
    response = client.post(
