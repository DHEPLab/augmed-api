...
def test_successful_file_upload(client, mocker):
    data = {"file": (BytesIO(b"content of the file"), "test.csv")}
    mocker.patch(
        "src.user.service.configuration_service.ConfigurationService.process_csv_file",
    ...
    response = client.post(
        "admin/config/upload", content_type="multipart/form-data", data=data
    )
...
def test_invalid_file_extension(client, mocker):
    data = {"file": (BytesIO(b"content of the file"), "test.txt")}
    mocker.patch(
        "src.user.utils.auth_utils.validate_jwt_and_refresh", return_value=None
    )
