...
def jwt_request_context(app, mocker, generate_dummy_jwt):
    with app.app_context():
        ...
        mocker.patch(
            "flask_jwt_extended.get_jwt_identity", return_value="test@example.com"
        )
        mocker.patch(
...
def test_validate_jwt_expired_new_token_issued(app, jwt_request_context, mocker, user):
    mocker.patch(
        "src.user.utils.auth_utils.get_jwt",
        return_value={
...
def test_validate_jwt_expired_last_login_over_3_days(
    app, jwt_request_context, mocker, user
):
    mocker.patch(
        "src.user.utils.auth_utils.get_jwt",
        return_value={
...
def test_validate_jwt_verification_fails(app, jwt_request_context, mocker):
    mocker.patch(
        "src.user.utils.auth_utils.verify_jwt_in_request",
        side_effect=Unauthorized("Invalid JWT"),
    )
    with pytest.raises(Unauthorized):
        validate_jwt_and_refresh()
